...
async function main() {
  ...
  check("10시 마감(remaining=0)", byHour[10] === 0, `10=${byHour[10]}`);
  check("9시 마감(remaining=0, 두 차량 출발준비로 막힘)", byHour[9] === 0, `9=${byHour[9]}`);
  check("11시 마감(remaining=0, 도착측 t+1로 막힘)", byHour[11] === 0, `11=${byHour[11]}`);
  check("12시 마감(remaining=0, 12시 출발준비=11시 막힘)", byHour[12] === 0, `12=${byHour[12]}`);
  check("13시 가능(remaining=4, 점유 영향권 벗어남)", byHour[13] === 4, `13=${byHour[13]}`);
