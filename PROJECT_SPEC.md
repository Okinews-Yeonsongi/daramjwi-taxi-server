...
- `이장님_최종본.rtf` 또는 `admin-prototype.html` — 이장님용 관리자 화면 프로토타입 (HTML/CSS)
...
4. **이장님 프로토타입 분석**: RTF 또는 HTML 파일을 열어보고 어떤 화면이 들어있는지 파악
...
충북 옥천군 청산면 마을 공동 택시(2대 운영, 청산면↔옥천읍 셔틀)를 주민이 스마트폰 앱웹으로 신청하고, 이장님이 확정/취소하면 주민에게 SMS가 발송되는 시스템.
...
```
[waiting] ──이장님 확정──> [confirmed] ──운행시간 도래──> [completed]
    │                          │
    ├──본인/이장님 취소──> [cancelled]
    └──본인/이장님 취소──> [cancelled]
```
...
| waiting → cancelled (이장님) | - | - | 주민에게 (사유 포함) |
| waiting → cancelled (본인) | - | - | 없음 |
| confirmed → cancelled (이장님) | +1 | -1 | 주민에게 (사유 포함) |
| confirmed → cancelled (본인) | +1 | -1 | 이장님에게 |
...
[다람쥐택시] 이장님, {이름}님이 {월}월 {일}일({요일}) {시간} 예약을 취소했어요.
노선: {출발}→{도착}, 인원: {N}명.
```
...
8. 모든 신청은 waiting 상태로 시작, 이장님 확정 필요
9. 운행 시작 시각 이후 취소 불가
